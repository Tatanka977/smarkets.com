            ...
            <div key={ex.code} style={{ ...CARD, padding: "12px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                  ...
                  <span style={{ fontSize: 14, fontWeight: 700, color: B.gray1, fontFamily: FONT }}>{ex.label}</span>
                </div>
                <div style={{ fontSize: 13, color, fontWeight: 700, fontFamily: FONT }}>
                  {s?.holiday ? "HOLIDAY" : isOpen ? "OPEN" : "CLOSED"}
                </div>
                <div style={{ fontSize: 12, color: B.gray3, fontFamily: FONT }}>{fmtLocal(s?.timezone)} local</div>
              </div>
...
function StatField({ label, value, sub, color }: any) {
  return (
    <div>
      <div style={{ fontSize: 11, color: B.gray3, letterSpacing: "0.06em", textTransform: "uppercase", fontFamily: FONT, marginBottom: 4 }}>{label}</div>
      <div style={{ fontSize: 20, fontWeight: 700, color: color || B.gray1, fontFamily: FONT }}>{value}</div>
      {sub && <div style={{ fontSize: 13, color: B.gray3, fontFamily: FONT }}>{sub}</div>}
    </div>
  );
}
  ...
  return (
    <div style={{ ...CARD, padding: "16px 18px" }}>
      ...
      {loading ? (
        ...
        <div style={{ height: 180, display: "flex", alignItems: "center", justifyContent: "center", background: B.panel2, borderRadius: 8 }}>
          <div style={{ fontSize: 13, color: B.gray3, fontFamily: FONT, textAlign: "center", padding: "0 20px", lineHeight: 1.6 }}>
        ...
        <div style={{ height: 280, width: "100%" }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={filtered} margin={{ top: 8, right: 16, bottom: 8, left: 0 }}>
              <XAxis dataKey="label" tick={{fontSize:12,fill:B.gray3}} minTickGap={50} axisLine={{stroke:B.border}} tickLine={false}/>
              <YAxis tick={{fontSize:12,fill:B.gray3}} tickFormatter={(v)=>`${v.toFixed(0)}%`} axisLine={false} tickLine={false} width={45}/>
              <Tooltip formatter={(v:any)=>v!=null?`${v.toFixed(2)}%`:"—"} contentStyle={{fontFamily:FONT,fontSize:13,borderRadius:8}}/>
  ...
  return (
    <div style={{ ...CARD, padding: "16px 18px" }}>
      ...
      {loading ? (
        ...
        <div style={{ fontSize: 13, color: B.gray3, fontFamily: FONT, padding: "10px 0" }}>No discussions yet — be the first to write one.</div>
      ) : (
        posts.map((p: any) => (
          <div key={p.id} onClick={() => openPost(p.id)} style={{ padding: "8px 0", borderTop: `1px solid ${B.border}`, cursor: "pointer" }}>
            <div style={{ fontSize: 14, color: B.gray1, fontFamily: FONT, lineHeight: 1.4, marginBottom: 3 }}>{p.title}</div>
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 14, background: B.bg }}>
      ...
      <div style={{ textAlign: "right" }}>
        <button onClick={onRefresh} disabled={refreshing || !holdings.length} style={{
          ...
          fontFamily: FONT, fontSize: 13, cursor: (refreshing || !holdings.length) ? "not-allowed" : "pointer", padding: "4px 10px", borderRadius: 6,
