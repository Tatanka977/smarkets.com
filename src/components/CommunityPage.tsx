...
const backBtnStyle: any = {
  ...
  fontFamily: FONT, fontSize: 13, fontWeight: 700, padding: 0, letterSpacing: "0.04em",
};

const primaryBtnStyle = (enabled: boolean): any => ({
  background: enabled ? B.blue : B.panel2,
  color: enabled ? B.white : B.gray3,
  border: "none", padding: "8px 16px", borderRadius: 6, fontFamily: FONT, fontSize: 13, fontWeight: 700,
...
function VoteControl({ score, myVote, disabled, onVote }: {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2, minWidth: 34, flexShrink: 0 }}>
      ...
      <span style={{ fontSize: 14, fontWeight: 700, fontFamily: FONT, color: score > 0 ? B.green : score < 0 ? B.red : B.gray2 }}>{score}</span>
...
function PostTypeTag({ type }: { type: CommunityPostType }) {
  ...
  return (
    <span style={{
      ...
      fontSize: 11, fontWeight: 700, fontFamily: FONT, letterSpacing: "0.03em", whiteSpace: "nowrap",
    }}>{meta.label}</span>
  );
}
...
const actionBtnStyle: any = {
  ...
  fontFamily: FONT, fontSize: 12, fontWeight: 700, padding: "4px 6px", borderRadius: 6,
};
        ...
        {menuOpen && (
          <div style={{
            ...
            <button onClick={(e) => { e.stopPropagation(); setMenuOpen(false); }} style={{
              ...
              color: B.gray1, cursor: "pointer", fontFamily: FONT, fontSize: 13, padding: "8px 12px",
            }}>Report</button>
            <button onClick={(e) => { e.stopPropagation(); setMenuOpen(false); }} style={{
              display: "block", width: "100%", textAlign: "left", background: "none", border: "none",
              color: B.gray1, cursor: "pointer", fontFamily: FONT, fontSize: 13, padding: "8px 12px",
...
function MiniKpi({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div style={{ background: B.panel2, border: `1px solid ${B.border}`, borderRadius: 8, padding: "8px 10px", flex: "1 1 92px", minWidth: 92 }}>
      <div style={{ fontSize: 10, color: B.gray3, letterSpacing: "0.06em", fontFamily: FONT, marginBottom: 3 }}>{label}</div>
...
function AllocationBars({ title, data }: { title: string; data?: { name: string; value: number; pct: number }[] }) {
  ...
  return (
    <div style={{ flex: "1 1 200px", minWidth: 180 }}>
      <div style={{ fontSize: 11, color: B.gray3, letterSpacing: "0.06em", fontFamily: FONT, marginBottom: 8 }}>{title}</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {data.slice(0, 6).map((d, i) => (
          <div key={d.name}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: B.gray1, fontFamily: FONT, marginBottom: 2 }}>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, color: B.blue }}>
        <BarChartIcon />
        <span style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.08em", fontFamily: FONT }}>SHARED PORTFOLIO</span>
        <span style={{ fontSize: 12, color: B.gray3, fontFamily: FONT, fontWeight: 400 }}>· {snapshot.sourceName}</span>
      </div>
      <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginBottom: 14, flexWrap: "wrap" }}>
        ...
        <span style={{ fontSize: 14, fontWeight: 700, fontFamily: FONT, color: pCol(wDay) }}>
          {wDay >= 0 ? "+" : ""}{wDay.toFixed(2)}% today
        </span>
        <span style={{ fontSize: 12, color: B.gray3, fontFamily: FONT }}>{snapshot.holdings.length} positions</span>
      </div>
      ...
      <div style={{ fontSize: 11, color: B.gray3, letterSpacing: "0.06em", fontFamily: FONT, marginBottom: 8 }}>TOP HOLDINGS</div>
      <div style={{ display: "flex", gap: 18, flexWrap: "wrap", alignItems: "center", marginBottom: 18 }}>
        ...
        <table style={{ flex: "1 1 180px", borderCollapse: "collapse", fontFamily: FONT }}>
          <tbody>
            {chartRows.map((row, i) => (
              <tr key={row.name}>
                <td style={{ padding: "3px 0", fontSize: 13, color: B.gray1 }}>
                  <span style={{ width: 8, height: 8, borderRadius: 2, background: PIE_COLS[i % PIE_COLS.length], display: "inline-block", marginRight: 6 }} />
                  {row.name}
                </td>
                <td style={{ padding: "3px 0", fontSize: 13, color: B.gray1, textAlign: "right", fontWeight: 700 }}>{row.value.toFixed(1)}%</td>
                <td style={{ padding: "3px 0 3px 10px", fontSize: 12, textAlign: "right", color: row.change == null ? B.gray3 : row.change >= 0 ? B.green : B.red }}>
      ...
      {alerts.length > 0 && (
        <>
          <div style={{ fontSize: 11, color: B.gray3, letterSpacing: "0.06em", fontFamily: FONT, marginBottom: 8 }}>RISK ALERTS</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {alerts.map((a, i) => {
              ...
              return (
                <div key={i} style={{ border: `1px solid ${style.border}`, background: style.bg, borderRadius: 8, padding: "8px 10px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, marginBottom: 3 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: style.text, fontFamily: FONT }}>{style.icon} {a.title}</span>
                    <span style={{ fontSize: 12, fontWeight: 700, color: style.text, fontFamily: FONT, flexShrink: 0 }}>{a.metric}</span>
                  </div>
                  <div style={{ fontSize: 12, color: B.gray2, fontFamily: FONT, lineHeight: 1.4 }}>{a.detail}</div>
                </div>
              );
            })}
          </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {top.map((d, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 3 }}>
              ...
              <span style={{ fontSize: 11, color: B.gray2, fontFamily: FONT, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{d.name}</span>
              <span style={{ fontSize: 11, fontWeight: 700, color: B.gray1, fontFamily: FONT }}>{d.pct.toFixed(0)}%</span>
            </div>
          ))}
        </div>
      </div>
...
export function PortfolioBadge({ snapshot }: { snapshot: PortfolioSnapshot }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: 12, fontWeight: 700, fontFamily: FONT }}>Portfolio attached</span>
      <span style={{ fontSize: 12, color: B.gray3, fontFamily: FONT, fontWeight: 400 }}>
        · ${snapshot.totalValue.toLocaleString(undefined, { maximumFractionDigits: 0 })} · {snapshot.holdings.length} positions
      </span>
      {highRisk && <span style={{ fontSize: 12, color: SEV_STYLE.HIGH.text, fontWeight: 700, fontFamily: FONT }}>{SEV_STYLE.HIGH.icon} risk flag</span>}
    </div>
  );
}
  ...
  if (!open) {
    return (
      <button type="button" onClick={() => setOpen(true)} style={{
        ...
        padding: "6px 10px", fontFamily: FONT, fontSize: 12, fontWeight: 700, cursor: "pointer",
      }}>
  ...
  return (
    <div style={{ background: B.panel2, border: `1px solid ${B.border}`, borderRadius: 8, padding: 10, display: "flex", flexDirection: "column", gap: 6 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: 12, color: B.gray2, fontFamily: FONT, fontWeight: 700, letterSpacing: "0.04em" }}>ATTACH A PORTFOLIO</span>
        <button type="button" onClick={() => { setOpen(false); setSource(""); onChange(null); }} style={{
          background: "none", border: "none", color: B.gray3, cursor: "pointer", fontFamily: FONT, fontSize: 12, fontWeight: 700,
        }}>REMOVE</button>
      </div>
      {!hasCurrent && !(saved && saved.length) ? (
        <div style={{ fontSize: 12, color: B.gray3, fontFamily: FONT }}>Add positions in Portfolio first to share one.</div>
      ) : (
...
function UsernamePrompt({ onSet }: { onSet: (username: string) => void }) {
  ...
  return (
    <div style={{ background: B.panel, border: `1px solid ${B.blue}`, borderRadius: 12, padding: 14, display: "flex", flexDirection: "column", gap: 8 }}>
      <div style={{ fontSize: 13, color: B.gray2, fontFamily: FONT, lineHeight: 1.5 }}>
      ...
      {err && <div style={{ fontSize: 12, color: B.red, fontFamily: FONT }}>{err}</div>}
    </div>
  );
}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
            ...
            {showChannel && post.community_channels && (
              <span style={{ fontSize: 12, color: B.blue, fontWeight: 700, fontFamily: FONT }}>{post.community_channels.name}</span>
            )}
          </div>
          <div style={{ fontSize: 14, fontWeight: 700, color: B.gray1, fontFamily: FONT, marginBottom: 4 }}>{post.title}</div>
          <div style={{
            fontSize: 13, color: B.gray2, fontFamily: FONT, marginBottom: 6,
            overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box",
            WebkitLineClamp: 2, WebkitBoxOrient: "vertical" as any,
          }}>{post.body}</div>
          <div style={{ fontSize: 12, color: B.gray3, fontFamily: FONT, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
...
function Feed({ user, username, isAdmin, holdings, onUsernameSet, onOpenPost, selectedChannel, onSelectChannel }: {
  ...
  if (channelsLoading) {
    return <div style={{ textAlign: "center", padding: 30, color: B.gray3, fontFamily: FONT, fontSize: 14 }}>LOADING...</div>;
  }
    ...
    <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: 14, alignItems: "flex-start" }}>
      <div style={{ flex: 1, minWidth: 0, width: "100%" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8, marginBottom: 12 }}>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {(["hot", "new", "top", "following"] as const).map((t) => (
              <button key={t} onClick={() => setSortTab(t)} style={{
                ...
                borderRadius: 999, padding: "6px 14px", fontFamily: FONT, fontSize: 13, fontWeight: 700,
          ...
          <div ref={dropdownRef} style={{ position: "relative" }}>
            <button onClick={() => setDropdownOpen((v) => !v)} style={{
              ...
              borderRadius: 8, padding: "7px 12px", color: B.gray1, fontFamily: FONT, fontSize: 13, fontWeight: 700, cursor: "pointer",
            }}>
              ...
              <div style={{
                ...
                <div style={{ padding: 8, borderBottom: `1px solid ${B.border}` }}>
                  <input
                    ...
                    placeholder="Search communities…" style={{ ...inputStyle, width: "100%", fontSize: 13, padding: "6px 8px" }}
                  />
                </div>
                <button onClick={() => { setSelectedChannel(null); setDropdownOpen(false); }} style={{
                  ...
                  color: B.gray1, cursor: "pointer", fontFamily: FONT, fontSize: 13, fontWeight: 700, padding: "10px 12px",
                }}>All Topics</button>
                {filteredChannels.length === 0 ? (
                  <div style={{ padding: "12px", fontSize: 12, color: B.gray3, fontFamily: FONT }}>
                  ...
                  <button key={c.id} onClick={() => { setSelectedChannel(c); setDropdownOpen(false); }} style={{
                    ...
                    {c.description && (
                      <div style={{ fontSize: 11, color: B.gray3, marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.description}</div>
                    )}
                  </button>
                ))}
                <div style={{ borderTop: `1px solid ${B.border}` }}>
                  {user ? (
                    <button onClick={() => setShowCreateChannel((v) => !v)} style={{
                      display: "block", width: "100%", textAlign: "left", background: "none", border: "none",
                      color: B.blue, cursor: "pointer", fontFamily: FONT, fontSize: 13, fontWeight: 700, padding: "10px 12px",
                    }}>+ Create Topic</button>
                  ) : (
                    <Link to="/auth" style={{ display: "block", padding: "10px 12px", color: B.blue, fontFamily: FONT, fontSize: 13, fontWeight: 700, textDecoration: "none" }}>
        ...
        {!showForm && (
          <button onClick={openComposer} style={{
            ...
            <span style={{ flex: 1, fontSize: 14, color: B.gray3, fontFamily: FONT }}>
        ...
        {showCreateChannel && user && (
          username === null ? (
            ...
            <div style={{ background: B.panel, border: `1px solid ${B.border}`, borderRadius: 12, padding: 14, marginBottom: 12, display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <div>
                <div style={{ fontSize: 11, color: B.gray3, fontFamily: FONT, marginBottom: 4 }}>RULES (optional, one per line) — you decide these as the topic's creator</div>
        ...
        {showForm && (
          !user ? (
            <div style={{ background: B.panel, border: `1px solid ${B.border}`, borderRadius: 12, padding: 14, marginBottom: 12 }}>
              <Link to="/auth" style={{ color: B.blue, fontFamily: FONT, fontSize: 13, fontWeight: 700, textDecoration: "none" }}>Sign in to post →</Link>
            </div>
          ) : username === null ? (
            <div style={{ marginBottom: 12 }}><UsernamePrompt onSet={onUsernameSet} /></div>
          ) : (
            <div style={{ background: B.panel, border: `1px solid ${B.border}`, borderRadius: 12, padding: 14, marginBottom: 12, display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: 13, color: B.gray2, fontFamily: FONT, fontWeight: 700, letterSpacing: "0.04em" }}>NEW POST</span>
                <button onClick={() => setShowForm(false)} style={{ background: "none", border: "none", color: B.gray3, cursor: "pointer", fontFamily: FONT, fontSize: 12, fontWeight: 700 }}>CANCEL</button>
              </div>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {(Object.keys(POST_TYPE_META) as CommunityPostType[]).map((t) => (
                  <button key={t} type="button" onClick={() => setPostType(t)} style={{
                    padding: "5px 12px", borderRadius: 999, fontFamily: FONT, fontSize: 12, fontWeight: 700, cursor: "pointer",
        ...
        {error && (
          <div style={{ padding: "8px 10px", fontSize: 13, color: B.red, border: `1px solid ${B.red}`, borderRadius: 6, marginBottom: 10, fontFamily: FONT }}>
            {error}
          </div>
        )}

        {loading ? (
          <div style={{ textAlign: "center", padding: 30, color: B.gray3, fontFamily: FONT, fontSize: 14 }}>LOADING...</div>
        ) : isFollowingTab && !user ? (
          <div style={{ textAlign: "center", padding: 40, color: B.gray3, fontFamily: FONT, fontSize: 14, lineHeight: 1.6 }}>
          ...
          <div style={{ textAlign: "center", padding: 40, color: B.gray3, fontFamily: FONT, fontSize: 14 }}>
      ...
      <div style={{ width: isMobile ? "100%" : 300, flexShrink: 0, display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ background: B.panel, border: `1px solid ${B.border}`, borderRadius: 12, padding: 14 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: B.gray1, fontFamily: FONT, marginBottom: 10 }}>About {communityName}</div>
          {about ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <div style={{ fontSize: 13, color: B.gray3, fontFamily: FONT }}>Loading…</div>
          )}
          {selectedChannel && user && (
            <button disabled={followBusy} onClick={toggleFollow} style={{
              ...
              borderRadius: 6, padding: "6px 10px", fontFamily: FONT, fontSize: 12, fontWeight: 700,
          ...
          {selectedChannel && user && (user.user_id === selectedChannel.created_by || isAdmin) && (
            <button onClick={deleteSelectedChannel} style={{
              ...
              padding: "6px 10px", fontFamily: FONT, fontSize: 12, fontWeight: 700, cursor: "pointer", width: "100%",
            }}>DELETE THIS TOPIC</button>
          )}
        </div>

        <div style={{ background: B.panel, border: `1px solid ${B.borderB}`, borderRadius: 12, padding: 14 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: B.gray1, fontFamily: FONT, marginBottom: 10 }}>Trending Topics</div>
          {trending === null ? (
            <div style={{ fontSize: 13, color: B.gray3, fontFamily: FONT }}>Loading…</div>
          ) : totalTrendingCount < 5 ? (
            <div style={{ fontSize: 13, color: B.gray3, fontFamily: FONT, lineHeight: 1.5 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {trending.map((t, i) => (
                <div key={t.id} onClick={() => { const c = channels.find((ch) => ch.id === t.id); if (c) setSelectedChannel(c); }} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer" }}>
                  <span style={{ fontSize: 13, color: B.gray1, fontFamily: FONT }}>{i + 1}. {t.name}</span>
                  <span style={{ fontSize: 12, color: B.gray3, fontFamily: FONT }}>{t.count} posts</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ background: B.panel, border: `1px solid ${B.border}`, borderRadius: 12, padding: 14 }}>
          <button onClick={() => setRulesOpen((v) => !v)} style={{
            ...
            <span style={{ fontSize: 14, fontWeight: 700, color: B.gray1, fontFamily: FONT }}>
            ...
            !selectedChannel ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <textarea
                  ...
                  placeholder={"One rule per line"} style={{ ...inputStyle, width: "100%", resize: "vertical", fontSize: 13 }}
                />
                <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                  <button onClick={() => setEditingRules(false)} style={{ background: "none", border: "none", color: B.gray3, cursor: "pointer", fontFamily: FONT, fontSize: 12, fontWeight: 700 }}>CANCEL</button>
              ...
              <>
                {(selectedChannel.rules || []).length === 0 ? (
                  <div style={{ fontSize: 13, color: B.gray3, fontFamily: FONT, lineHeight: 1.5 }}>No rules set for this topic yet.</div>
                ) : (
                  <ol style={{ margin: 0, paddingLeft: 18, display: "flex", flexDirection: "column", gap: 8 }}>
                    {selectedChannel.rules.map((r, i) => (
                      <li key={i} style={{ fontSize: 13, color: B.gray2, fontFamily: FONT, lineHeight: 1.4 }}>{r}</li>
                    ))}
                  </ol>
                )}
...
function PostDetail({ postId, user, username, isAdmin, onUsernameSet, onBack }: {
  ...
  if (!post) {
    return (
      <div>
        ...
        <div style={{ padding: 20, color: B.gray3, fontFamily: FONT, fontSize: 14 }}>Discussion not found.</div>
      </div>
    );
  }
  ...
  return (
    <div>
      <button onClick={onBack} style={backBtnStyle}>← BACK</button>
      {error && (
        <div style={{ padding: "8px 10px", fontSize: 13, color: B.red, border: `1px solid ${B.red}`, borderRadius: 6, margin: "10px 0", fontFamily: FONT }}>
          {error}
        </div>
      )}
      <div style={{ background: B.panel, border: `1px solid ${B.borderB}`, borderRadius: 12, padding: "14px 16px", display: "flex", gap: 14, marginTop: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: B.gray3, fontFamily: FONT, marginBottom: 10, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <Avatar name={post.author_name} size={18} />
            <span>u/{post.author_name}</span>
            <span>· {fmtDate(post.created_at)}</span>
            {user && (user.user_id === post.user_id || isAdmin) && (
              <button onClick={removePost} style={{ background: "none", border: "none", color: B.red, cursor: "pointer", fontFamily: FONT, fontSize: 12, fontWeight: 700, padding: 0, marginLeft: "auto" }}>
                DELETE
              </button>
            )}
          </div>
          <div style={{ fontSize: 14, color: B.gray1, fontFamily: FONT, whiteSpace: "pre-wrap", lineHeight: 1.5 }}>{post.body}</div>
      ...
      <div id="community-comments" style={{ marginTop: 16 }}>
        ...
        {user ? (
          ...
          <div style={{ fontSize: 13, color: B.gray3, fontFamily: FONT, marginBottom: 14 }}>
            <Link to="/auth" style={{ color: B.blue, fontWeight: 700, textDecoration: "none" }}>Sign in</Link> to leave a comment.
          </div>
        )}

        {comments.length === 0 ? (
          <div style={{ fontSize: 13, color: B.gray3, fontFamily: FONT, padding: "10px 0" }}>No comments yet.</div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {comments.map((c) => (
              <div key={c.id} style={{ background: B.panel, border: `1px solid ${B.border}`, borderRadius: 10, padding: "10px 12px", display: "flex", gap: 10 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12, color: B.gray3, fontFamily: FONT, marginBottom: 4, display: "flex", gap: 8, alignItems: "center" }}>
                    ...
                    {user && (user.user_id === c.user_id || isAdmin) && (
                      <button onClick={() => removeComment(c.id)} style={{ background: "none", border: "none", color: B.red, cursor: "pointer", fontFamily: FONT, fontSize: 12, fontWeight: 700, padding: 0, marginLeft: "auto" }}>
                        DELETE
                      </button>
                    )}
                  </div>
                  <div style={{ fontSize: 14, color: B.gray1, fontFamily: FONT, whiteSpace: "pre-wrap" }}>{c.body}</div>
                </div>
              </div>
            ))}
          </div>
